function printTens () {
    for (let i = 10; i <= 100; i+=10 ) {
        console.log(i);
    }
}

//printTens();

function countUntilTens (step) {
    let i = step;
    while (true) {
        console.log(i);

        i += step;

        if (i % 10 === 0) {
            console.log(i);
            break;
        }        
    }
}
//countUntilTens(4);

function hasVowels (word) {

        for (let i=0; i <word.length; i++) {
            console.log(word[i]);
            if (word[i] === 'a' || word[i] === 'e' || word[i] === 'i'|| word[i] === 'o' || word[i] === 'u') {
                return true;
            }
         }
         return false;
    }
//

function testTemperature(desiredTemp, actualTemp) {
    if (actualTemp > desiredTemp) {
        console.log('Run AC')
    }
    else if (actualTemp < desiredTemp) {
        console.log('Run Heat')
    }
    else {console.log('Standby')}

}

//testTemperature(85,95);


const dealership = [
    {
        make: 'Toyota',
        model: 'Prius',
        year:  2020,
        hybrid: true,
    },
    {
        make: 'Toyota',
        model: 'Prius',
        year:  2021,
        hybrid: true,     
    },
    {
        make: 'Ford',
        model: 'F-150',
        year:  2021,
        hybrid: false,
    },
    {
        make: 'Ferrari',
        model: 'Enzo',
        year:  2004,
        hybrid: false,
    },

]

function findCars (arrayOfCars, model) {
    return arrayOfCars.filter(found => found.model === model)
}

//console.log(findCars(dealership, "Prius"));
 
function addCar (arrayOfCars,newMake,newModel,newHybrid) {
    const newCar = {
        make:newMake,
        model: newModel,
        year: 2021,
        hybrid: newHybrid,
     };

    arrayOfCars.unshift(newCar);
}
//addCar(dealership, "Honda", "Accord", "True",);
//console.log(dealership);

function areThereAnyHybridsForSale(arrayOfCars) {
    return arrayOfCars.some(cars =>cars.hybrid === true);
}
//console.log(areThereAnyHybridsForSale(dealership));




